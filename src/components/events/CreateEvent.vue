  ...
  <el-dialog :title="updatingEvent ? 'Update Event' : 'Create Event'" :visible.sync="showCreateEvent">
    <div class="create-event-form">
      <el-form label-position="top" class="create-event">
        <ImageUploader :placeHolder="backgroundImage" imageType="events" v-model="eventInfo.avatarUrl">
        </ImageUploader>
        <div class="create-event-input">
          ...
          <el-form-item label="Location">
            <gmap-autocomplete class="google-maps-autocomplete" @place_changed="onSetLocation">
            </gmap-autocomplete>
          </el-form-item>
          ...
          <el-form-item label="Tags">
            <el-tag :key="tag.tagname" v-for="tag in eventInfo.tags" closable :disable-transitions="false" @close="handleClose(tag)">
              {{tag.tagname}}
            </el-tag>
            <el-autocomplete class="input-new-tag" v-if="inputVisible" v-model="inputValue.tagname" :fetch-suggestions="querySearch"
              :trigger-on-focus="false" ref="saveTagInput" size="mini" @keyup.enter.native="handleInputConfirm" @select="handleSelect">
              <i class="el-icon-close el-input__icon" slot="suffix" @click="handleCloseEdit">
              </i>
            </el-autocomplete>
