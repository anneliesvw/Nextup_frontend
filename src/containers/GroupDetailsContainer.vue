<template>
  <div class="group-detail-container main-container">
    <CreateEvent v-if="eventDialogVisible" :isVisible="true" :activeGroup="activeGroup" @close="closeEventDialog">
    </CreateEvent>
    <MemberDialog v-if="dialogVisible" :isVisible="true" @close="closeDialog" :activeGroup="activeGroup">
    </MemberDialog>
    <GroupDialog v-if="groupDialogVisible" :isVisible="true" @close="closeGroupDialog">
    </GroupDialog>
    <CreatePoll v-if="pollDialogVisible" :activeGroup="this.activeGroup" :isVisible="true" @close="closePollDialog">
    </CreatePoll>
    <PollDialog :poll="this.pollDetail" v-if="pollDetailVisible" :isVisible="true" :activeGroup="this.activeGroup" @close="closePollDetail">
    </PollDialog>

    <div class="sidebar-wrapper">
      <GroupSidebar @groupSelected="onGroupSelected" @createClicked="onCreateGroup" :activeGroup="activeGroup">
      </GroupSidebar>
    </div>
    <div class="group-wall" v-if="activeGroup">
      ...
      <div class="group-panels">
        ...
        <PollInfoPanel title="Polls" :polls="activeGroup.polls" @showPollDetail="showPollDetail($event)" @showCreatePoll="setPollDialogVisible">
        </PollInfoPanel>
      </div>
    </div>
    <div class="no-group-selected" v-else>
      <EmptyState size="large" title="No Group Selected" text="please select a group through the menu on the left side" icon="hand-point-left">
